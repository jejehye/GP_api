...
#### `/send/v1/account` 비밀번호 암호화 예시

API 요청 비밀번호가 `0000`인 경우 하나의 요청에서 다음과 같이 각각 암호화합니다.
...
GP 전송 값은 고정 마스크 `'K'`를 사용하므로 항상 같습니다.
...
GMSH 전송 값은 랜덤 마스크를 사용합니다. 예를 들어 마스크로 `'K'`가 선택되면
암호화 결과는 다음과 같습니다.
...
GMSH 값의 마지막 문자 `K`는 암호화에 사용된 마스크입니다. 다른 마스크가
선택되면 `acct_pwd` 값도 달라지므로 위 값은 가능한 결과 중 하나입니다.
